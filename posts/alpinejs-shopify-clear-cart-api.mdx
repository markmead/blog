...
<Callout>
  If using `assets/theme.js` then you will need to import Alpine JS as a module.
</Callout>
...
<Callout>
  If the clear cart button is within a `form` then you'll need to add
  `type="button"` to the button to prevent it from submitting the form.
</Callout>
